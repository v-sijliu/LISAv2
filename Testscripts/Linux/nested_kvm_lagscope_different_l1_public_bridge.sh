...
#######################################################################
#
# nested_kvm_lagscope_different_l1_public_bridge.sh
# Description:
#   This script runs lagscope test on two nested VMs on different L1 guests connected with public bridge
#
#######################################################################
...
if [ -z "$pingIteration" ]; then
        echo "Please mention -pingIteration next"
        exit 1
fi
if [ -z "$logFolder" ]; then
        logFolder="."
